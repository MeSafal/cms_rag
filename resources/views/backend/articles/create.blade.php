        ...
        <div class="row g-4">
            <div class="col-sm-12 col-xl-8">
                <div class="rounded h-100 p-4">
                    ...
                    <h5 class="mb-4">
                        @if (isset($item))
                            {{ label('Edit article') }} <span class="text-primary">{{ $item->title }}</span>
                        @else
                            {{ label('Create article') }}
                        @endif
                    </h5>
                    <form
                        action="{{ isset($item) ? route('articles.update', $item->articles_id) : route('articles.store') }}"
                        ...
                        <div class="row">
                            ...
                            <div class="pt-4 col-xl-6 col-md-12">
                                <label for="subticovertle" class="form-label">{{ label('Cover Image') }}</label>
                                {!! CreateImage('cover', 'Cover Image', 'cover', 'holder', old('cover', isset($item) ? $item->cover : '')) !!}
                            </div>
                            <div class="pt-4 col-xl-6 col-md-12">
                                <label for="thumb" class="form-label">{{ label('Thumb Image') }}</label>
