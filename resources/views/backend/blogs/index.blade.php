                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Active Blogs</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Inactive Blogs</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Total Blogs</p>
                        </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="col-auto" style="padding: 5px;">
                        <select class="form-select form-select-sm" id="rowsDropdown"
                            ...
                            <option
                                value="{{ route('blogs.index', array_merge(request()->query(), ['rows' => 10])) }}"
                                {{ request('rows', 10) == 10 ? 'selected' : '' }}>10</option>
                            <option
                                value="{{ route('blogs.index', array_merge(request()->query(), ['rows' => 25])) }}"
                                {{ request('rows') == 25 ? 'selected' : '' }}>25</option>
                            <option
                                value="{{ route('blogs.index', array_merge(request()->query(), ['rows' => 50])) }}"
                                {{ request('rows') == 50 ? 'selected' : '' }}>50</option>
                            <option
                                value="{{ route('blogs.index', array_merge(request()->query(), ['rows' => 100])) }}"
                                {{ request('rows') == 100 ? 'selected' : '' }}>100</option>
                            {{-- <option
                                value="{{ route('blogs.index', array_merge(request()->query(), ['rows' => 500])) }}"
                                {{ request('rows') == 500 ? 'selected' : '' }}>500</option> --}}
                        </select>
                    </div>

                </div>
                <form id="update-order-form" action="{{ route('blogs.updateOrder') }}" method="POST"
                    style="display: none;">
                    @csrf
                    <input type="hidden" name="order" value="">
                </form>
                <div class="table-responsive">
                        <table class="table  table-borderless reorderable-table" id="blogs-table"
                        data-url="{{ route('blogs.updateOrder') }}"
                        data-permission="{{ auth()->user()->can('blogs.updateOrder') ? 'true' : 'false' }}">
                        ...
                        <tbody>
                            @foreach ($items as $item)
                                <tr data-id="{{ $item->blogs_id }}" data-order="{{ $item->display_order }}">
                                    ...
                                    @can('blogs.alias')
                                        <td>
                                            <span class="alias-text"
                                                ...
                                                data-url-template="{{ route('blogs.alias', ['id' => ':id']) }}"
                                        ...
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input page-toggle" type="checkbox"
                                                    ...
                                                    data-url-template="{{ route('blogs.publish', ['id' => $item->blogs_id, 'publish' => ':publish']) }}"
                                    ...
                                    @endcan
                                    @canany(['blogs.view', 'blogs.edit', 'blogs.publish',
                                        'blogs.delete'])
        ...
        $(document).ready(function() {
                ...
                typingTimer = setTimeout(function() {
                    ...
                    if (searchQuery !== searchQueryCache) {
                        ...
                        if (searchQueryCache === '') {
                            ...
                            window.location.href = `/admin/blogs${query}`;
                        } else {
            ...
            */
            function fetchTableData(searchQuery, rows) {
                $.ajax({
                    url: '{{ route('blogs.index') }}', // Laravel route
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        $('#blogs-table tbody').html(response.item_html);
                        ...
                        */
                        ...
                        bindAliasEditing();
                        bindPopupToggle();
                        bindDragAndDropForTable('blogs-table');
                        bindActionButtons();
                    },
