<!-- resources/views/backend/coachings/index.blade.php -->
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Active Coachings</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Inactive Coachings</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Total Coachings</p>
                        </div>
        ...
        <div class="col-12">
            <div class="rounded h-100 p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h6 class="mb-0">Coachings List</h6>
                    ...
                    <div class="col-auto" style="padding: 5px;">
                        <select class="form-select form-select-sm" id="rowsDropdown"
                            ...
                            <option
                                value="{{ route('coachings.index', array_merge(request()->query(), ['rows' => 10])) }}"
                                {{ request('rows', 10) == 10 ? 'selected' : '' }}>10</option>
                            <option
                                value="{{ route('coachings.index', array_merge(request()->query(), ['rows' => 25])) }}"
                                {{ request('rows') == 25 ? 'selected' : '' }}>25</option>
                            <option
                                value="{{ route('coachings.index', array_merge(request()->query(), ['rows' => 50])) }}"
                                {{ request('rows') == 50 ? 'selected' : '' }}>50</option>
                            <option
                                value="{{ route('coachings.index', array_merge(request()->query(), ['rows' => 100])) }}"
                                {{ request('rows') == 100 ? 'selected' : '' }}>100</option>
                            {{-- <option
                                value="{{ route('coachings.index', array_merge(request()->query(), ['rows' => 500])) }}"
                                {{ request('rows') == 500 ? 'selected' : '' }}>500</option> --}}
                        </select>
                    </div>

                </div>
                <form id="update-order-form" action="{{ route('coachings.updateOrder') }}" method="POST"
                    style="display: none;">
                    @csrf
                    <input type="hidden" name="order" value="">
                </form>
                <div class="table-responsive">
                        <table class="table  table-borderless reorderable-table" id="coachings-table"
                        data-url="{{ route('coachings.updateOrder') }}"
                        data-permission="{{ auth()->user()->can('coachings.updateOrder') ? 'true' : 'false' }}">
                        <thead class="table-light">
                            <tr>
                                ...
                                @canany(['coachings.view', 'coachings.edit', 'coachings.delete'])
                        ...
                        <tbody>
                            @foreach ($items as $item)
                                <tr data-id="{{ $item->coachings_id }}" data-order="{{ $item->display_order }}">
                                    <th scope="row"
                                        class="{{ auth()->user()->can('coachings.updateOrder') ? 'move-up-icon' : '' }}">
                                        {{ $loop->index + 1 }}</th>
                                    <td>{{ Str::limit($item->title, 50) }}</td>
                                    @can('coachings.alias')
                                        <td>
                                            <span class="alias-text"
                                                data-id="{{ $item->coachings_id }}"
                                                data-url-template="{{ route('coachings.alias', ['id' => ':id']) }}"
                                    ...
                                    @can('coachings.publish')
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input page-toggle" type="checkbox"
                                                    data-page-id="{{ $item->coachings_id }}"
                                                    data-url-template="{{ route('coachings.publish', ['id' => $item->coachings_id, 'publish' => ':publish']) }}"
                                    ...
                                    @endcan
                                    @canany(['coachings.view', 'coachings.edit', 'coachings.publish',
                                        'coachings.delete'])
                                        <td class="icon-wrapper">
                                            ...
                                            <div class="popup-container">
                                                @can('coachings.view')
                                                    <a href="{{ route('coachings.view', ['id' => $item->coachings_id]) }}"
                                                ...
                                                @endcan
                                                @can('coachings.edit')
                                                    <a href="{{ route('coachings.edit', ['id' => $item->coachings_id]) }}"
                                                ...
                                                @can('coachings.delete')
                                                    <a href="#" class="text-icon delete-btn"
                                                        data-action="delete"
                                                        data-url="{{ route('coachings.delete', ['id' => $item->coachings_id]) }}"
                                                        title="Delete">
                                                        <i style="font-size: 20px; padding-right: 10px;"
                                                ...
                                                {{-- @can('coachings.publish')
                                                    @if ($item->status == 1)
                                                        <a href="#" class="text-icon publish-btn"
                                                            ...
                                                            data-url="{{ route('coachings.publish', ['id' => $item->coachings_id, 'publish' => -1]) }}"
                                                            title="Unpublish">
                                                            <i class="fa fa-ban"
    ...
    function bindSearchInput() {
        $(document).ready(function() {
                ...
                typingTimer = setTimeout(function() {
                    ...
                    if (searchQuery !== searchQueryCache) {
                        ...
                        if (searchQueryCache === '') {
                            ...
                            window.location.href = `/admin/coachings${query}`;
                        } else {
            ...
            */
            function fetchTableData(searchQuery, rows) {
                $.ajax({
                    url: '{{ route('coachings.index') }}', // Laravel route
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        $('#coachings-table tbody').html(response.item_html);
                        ...
                        */
                        ...
                        bindAliasEditing();
                        bindPopupToggle();
                        bindDragAndDropForTable('coachings-table');
                        bindActionButtons();
                    },
        ...
        */
            ...
            function updateTableLocally(query) {
                $('#coachings-table tbody tr').each(function() {
