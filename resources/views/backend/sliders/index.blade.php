                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Active Sliders</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Inactive Sliders</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Total Sliders</p>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <div class="col-auto" style="padding: 5px;">
                                <select class="form-select form-select-sm" id="rowsDropdown"
                                    ...
                                    <option
                                        value="{{ route('sliders.index', array_merge(request()->query(), ['rows' => 10])) }}"
                                        {{ request('rows', 10) == 10 ? 'selected' : '' }}>10</option>
                                    <option
                                        value="{{ route('sliders.index', array_merge(request()->query(), ['rows' => 25])) }}"
                                        {{ request('rows') == 25 ? 'selected' : '' }}>25</option>
                                    <option
                                        value="{{ route('sliders.index', array_merge(request()->query(), ['rows' => 50])) }}"
                                        {{ request('rows') == 50 ? 'selected' : '' }}>50</option>
                                    <option
                                        value="{{ route('sliders.index', array_merge(request()->query(), ['rows' => 100])) }}"
                                        {{ request('rows') == 100 ? 'selected' : '' }}>100</option>
                                    {{-- <option
                                        value="{{ route('sliders.index', array_merge(request()->query(), ['rows' => 500])) }}"
                                        {{ request('rows') == 500 ? 'selected' : '' }}>500</option> --}}
                                </select>
                            </div>

                        </div>
                        <form id="update-order-form" action="{{ route('sliders.updateOrder') }}" method="POST"
                            style="display: none;">
                            @csrf
                            <input type="hidden" name="order" value="">
                        </form>
                        <div class="table-responsive">
                             <table class="table table-borderless reorderable-table" id="sliders-table"
                                data-url="{{ route('sliders.updateOrder') }}"
                                data-permission="{{ auth()->user()->can('sliders.updateOrder') ? 'true' : 'false' }}">
                                ...
                                <tbody>
                                    @foreach ($items as $item)
                                        <tr data-id="{{ $item->sliders_id }}" data-order="{{ $item->display_order }}">
                                            <th scope="row"
                                                class="{{ auth()->user()->can('sliders.updateOrder') ? 'move-up-icon' : '' }}">
                                                {{ $loop->index + 1 }}</th>
                                            <td>{{ Str::limit($item->title, 50) }}</td>
                                            @can('sliders.alias')
                                                <td>
                                                    <span class="alias-text"
                                                        data-id="{{ $item->sliders_id }}"
                                                        data-url-template="{{ route('sliders.alias', ['id' => ':id']) }}"
                                           ...
                                           @can('sliders.publish')
                                                <td>
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input page-toggle" type="checkbox"
                                                            data-page-id="{{ $item->sliders_id }}"
                                                            data-url-template="{{ route('sliders.publish', ['id' => $item->sliders_id, 'publish' => ':publish']) }}"
                                            ...
                                            @endcan
                                            @canany(['sliders.view', 'sliders.edit', 'sliders.publish',
                                                'sliders.delete'])
                                                <td class="icon-wrapper">
                                                    ...
                                                    <div class="popup-container">
                                                        @can('sliders.view')
                                                            <a href="{{ route('sliders.view', ['id' => $item->sliders_id]) }}"
                                                        ...
                                                        @endcan
                                                        @can('sliders.edit')
                                                            <a href="{{ route('sliders.edit', ['id' => $item->sliders_id]) }}"
                                                        ...
                                                        @can('sliders.delete')
                                                            <a href="#" class="text-icon delete-btn"
                                                                data-action="delete"
                                                                data-url="{{ route('sliders.delete', ['id' => $item->sliders_id]) }}"
                                                                title="Delete">
                                                                <i style="font-size: 20px; padding-right: 10px;"
        ...
        $(document).ready(function() {
                ...
                typingTimer = setTimeout(function() {
                    ...
                    if (searchQuery !== searchQueryCache) {
                        ...
                        if (searchQueryCache === '') {
                            ...
                            window.location.href = `/admin/sliders${query}`;
                        } else {
            ...
            */
            function fetchTableData(searchQuery, rows) {
                $.ajax({
                    url: '{{ route('sliders.index') }}', // Laravel route
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        $('#sliders-table tbody').html(response.item_html);
                        ...
                        */
                        ...
                        bindAliasEditing();
                        bindPopupToggle();
                        bindDragAndDropForTable('sliders-table');
                        bindActionButtons();
                    },
