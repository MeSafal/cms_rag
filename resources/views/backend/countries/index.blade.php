                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Active Countries</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Inactive Countries</p>
                        </div>
                ...
                <div class="card card-animate h-100">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            ...
                            <p class="mb-0 text-uppercase fw-medium text-muted">Total Countries</p>
                        </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="col-auto" style="padding: 5px;">
                        <select class="form-select form-select-sm" id="rowsDropdown"
                            ...
                            <option
                                value="{{ route('countries.index', array_merge(request()->query(), ['rows' => 10])) }}"
                                {{ request('rows', 10) == 10 ? 'selected' : '' }}>10</option>
                            <option
                                value="{{ route('countries.index', array_merge(request()->query(), ['rows' => 25])) }}"
                                {{ request('rows') == 25 ? 'selected' : '' }}>25</option>
                            <option
                                value="{{ route('countries.index', array_merge(request()->query(), ['rows' => 50])) }}"
                                {{ request('rows') == 50 ? 'selected' : '' }}>50</option>
                            <option
                                value="{{ route('countries.index', array_merge(request()->query(), ['rows' => 100])) }}"
                                {{ request('rows') == 100 ? 'selected' : '' }}>100</option>
                            {{-- <option
                                value="{{ route('countries.index', array_merge(request()->query(), ['rows' => 500])) }}"
                                {{ request('rows') == 500 ? 'selected' : '' }}>500</option> --}}
                        </select>
                    </div>

                </div>
                <form id="update-order-form" action="{{ route('countries.updateOrder') }}" method="POST"
                    style="display: none;">
                    @csrf
                    <input type="hidden" name="order" value="">
                </form>
                <div class="table-responsive">
                        <table class="table  table-borderless reorderable-table" id="countries-table"
                        data-url="{{ route('countries.updateOrder') }}"
                        data-permission="{{ auth()->user()->can('countries.updateOrder') ? 'true' : 'false' }}">
                        <thead class="table-light">
                            <tr>
                                ...
                                @canany(['countries.view', 'countries.edit', 'countries.delete'])
                        ...
                        <tbody>
                            @foreach ($items as $item)
                                <tr data-id="{{ $item->countries_id }}" data-order="{{ $item->display_order }}">
                                    ...
                                    @can('countries.alias')
                                        <td>
                                            <span class="alias-text"
                                                ...
                                                data-url-template="{{ route('countries.alias', ['id' => ':id']) }}"
                                        ...
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input page-toggle" type="checkbox"
                                                    ...
                                                    data-url-template="{{ route('countries.publish', ['id' => $item->countries_id, 'publish' => ':publish']) }}"
                                    ...
                                    @endcan
                                    @canany(['countries.view', 'countries.edit', 'countries.publish',
                                        'countries.delete'])
        ...
        $(document).ready(function() {
                ...
                typingTimer = setTimeout(function() {
                    ...
                    if (searchQuery !== searchQueryCache) {
                        ...
                        if (searchQueryCache === '') {
                            ...
                            window.location.href = `/admin/countries${query}`;
                        } else {
            ...
            */
            function fetchTableData(searchQuery, rows) {
                $.ajax({
                    url: '{{ route('countries.index') }}', // Laravel route
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        $('#countries-table tbody').html(response.item_html);
                        ...
                        */
                        ...
                        bindAliasEditing();
                        bindPopupToggle();
                        bindDragAndDropForTable('countries-table');
                        bindActionButtons();
                    },
