...
class BlogsController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('permission:blogs.index', only: ['index']),
            new Middleware('permission:blogs.create', only: ['form']),
            new Middleware('permission:blogs.edit', only: ['form']),
            new Middleware('permission:blogs.view', only: ['view']),
            new Middleware('permission:blogs.delete', only: ['delete']),
            new Middleware('permission:blogs.alias', only: ['alias']),
            new Middleware('permission:blogs.publish', only: ['publish']),
            new Middleware('permission:blogs.store', only: ['save']),
            new Middleware('permission:blogs.update', only: ['save']),
            new Middleware('permission:blogs.updateOrder', only: ['updateOrder']),
        ];
    }
    ...
    public function index(Request $request)
    {
        ...
        $inactiveCount = Blog::where('status', -1)->count();

        try {
            $items = Blog::activeStatus()
                ->search($search)
                ->ordered()
                ->paginate($rows);
            ...
            if ($request->ajax()) {
                ...
                $item_html = view('backend.blogs.table', compact('items'))->render();
            ...
            return view('backend.blogs.index', compact('items', 'activeCount', 'inactiveCount'));
        } catch (\Exception $e) {
    ...
    public function form($id = null)
    {
        ...
        $items = Blog::where('status', '<>', 0)
            //->where('parent', null)
            ->where('blogs_id', '<>',  $id ?? null)
            ->orderBy('title', 'asc')
            ->pluck('title', 'blogs_id')->toArray();

        // If $id is provided, fetch the item for editing
        $item = $id ? Blog::findOrFail($id) : null;
        ...
        $location = ($item
        ? $this->templateService->getTemplateLocation('Blog', $item->blogs_id)
        : null) ?? $this->templateService->getDefaultLocation('home', 'home.slider');

        return view('backend.blogs.create', compact('items', 'item', 'location'));
    }
    ...
    public function save(Request $request, $id = null)
    {
        ...
        $this->templateService->updateTemplateEntries('Blog', $templateId, $childId, $item->blogs_id, $item->status);

        // Redirect based on user role/permission
        if (Auth::user()->hasRole('superadmin') || Auth::user()->hasPermissionTo('blogs.index')) {
    ...
    {
        $item = Blog::findOrFail($id);
        return view('backend.blogs.view', compact('item'));
    }
